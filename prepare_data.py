...
for i in range(1,total_num + 1):
    for seq_num in range(num_in_seq):
        hr=np.zeros((n_frame,1,z_length,patchsize*factor,patchsize*factor)).astype(np.float64)
        lr=np.zeros((n_frame,1,z_length,patchsize,patchsize)).astype(np.float64)
        for k in range(n_frame):
            ...
            hr[k,::]=np.asarray(hr_temp).astype(np.float32)
            lr[k,::]=np.asarray(lr_temp).astype(np.float32)
